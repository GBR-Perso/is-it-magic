...
- **Never re-validate suite-produced input.** When a skill's input is a report or recommendation produced by another suite skill earlier in this session — whether printed directly to the conversation, or written to a file whose path a printed digest names (e.g. an Investigation Report feeding `/project-decide` via its digest's file, or a Decision Report recommendation feeding `/project-implement`) — present the restatement as plain text and proceed without asking; the user already approved that content: either when it was printed, or by choosing to continue past the digest that named it.
